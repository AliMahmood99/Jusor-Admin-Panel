...
import { colorVariants, avatarGradients } from '@/lib/designSystem';
...
export default function DisputePartyCard({ party, role }: DisputePartyCardProps) {
  ...
  const avatarClass = isInfluencer ? avatarGradients.influencer : avatarGradients.business;
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 p-6">
      ...
      <div className="flex items-center gap-4 mb-4">
        <div className={`w-14 h-14 rounded-2xl ${avatarClass} flex items-center justify-center text-white text-lg font-bold shadow-lg`}>
          {party.avatar || party.name.split(' ').map(n => n[0]).join('').slice(0, 2)}
        </div>
        <div>
          <h4 className="text-base font-semibold text-gray-900">{party.name}</h4>
          <p className={`text-sm ${colors.text}`}>ID: {party.id}</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 pt-4 border-t border-gray-100">
        ...
        <button className={`flex-1 h-9 rounded-lg ${colors.button} text-white text-sm font-medium transition-colors`}>
          Message
        </button>
      </div>
    </div>
